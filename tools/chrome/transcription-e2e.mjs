...
context.on('page',capturePageDiagnostics);
for(const existing of context.pages())capturePageDiagnostics(existing);
...
try{
  ...
  // A browser-action popup is not itself the active tab. Keep a real supported HTTP tab
  // active, then open popup.html in a background extension tab so GET_ACTIVE_STATE sees
  // the same relationship it sees for a real user popup.
  const workPage=await context.newPage();await workPage.goto(supportedUrl);await workPage.bringToFront();
  const popupUrl=`chrome-extension://${extensionId}/popup.html`;
  await serviceWorker.evaluate(url=>chrome.tabs.create({url,active:false}),popupUrl);
  let page=context.pages().find(item=>item.url()===popupUrl);
  if(!page){page=await context.waitForEvent('page',{predicate:item=>item.url()===popupUrl,timeout:10000});}
  await page.waitForLoadState('domcontentloaded');

  const popupJsState=await page.evaluate(async()=>{
    const script=[...document.scripts].find(item=>item.getAttribute('src')==='popup.js');
    let fetchOk=false;let status=0;try{const response=await fetch(chrome.runtime.getURL('popup.js'));status=response.status;fetchOk=response.ok;}catch{}
    return{scriptPresent:Boolean(script),scriptType:script?.type||'',fetchOk,status};
  });

  let initialized=false;
  try{
    await page.waitForFunction(()=>{
      const toggle=document.querySelector('#simpleToggle');
      const empty=document.querySelector('#emptyActions');
      return toggle instanceof HTMLButtonElement&&typeof toggle.onclick==='function'&&empty instanceof HTMLElement&&empty.hidden===false;
    },null,{timeout:5000});
    initialized=true;
  }catch{}

  const initState=await page.evaluate(()=>{
    const toggle=document.querySelector('#simpleToggle');const panel=document.querySelector('#simplePanel');const empty=document.querySelector('#emptyActions');const state=document.querySelector('#state');
    return{
      listenerType:toggle instanceof HTMLButtonElement?typeof toggle.onclick:'missing',
  ...
  if(!initialized){
    throw new Error(`popup_not_initialized:${JSON.stringify({popupJsState,initState,consoleErrors,pageErrors,failedRequests})}`);
  }

  // Now that load() has completed against the supported underlying work tab, interact
  // through Playwright with the real button and real compiled popup.js listener.
  await page.bringToFront();
  const toggle=page.locator('#simpleToggle');const panel=page.locator('#simplePanel');
  ...
  await toggle.click();await page.waitForFunction(()=>document.querySelector('#simpleToggle')?.getAttribute('aria-expanded')==='true'&&document.querySelector('#simplePanel') instanceof HTMLElement&&!document.querySelector('#simplePanel').hidden);
  disclosure.open=await readDisclosure();
  await toggle.click();await page.waitForFunction(()=>document.querySelector('#simpleToggle')?.getAttribute('aria-expanded')==='false'&&document.querySelector('#simplePanel') instanceof HTMLElement&&document.querySelector('#simplePanel').hidden);
  ...
  const result=await page.evaluate(async fixtureBase64=>{
    ...
    const logo=document.querySelector('.logo');const brand=document.querySelector('.identity span');
    let decodeOk=false;if(logo instanceof HTMLImageElement){try{await logo.decode();decodeOk=true;}catch{}}
    const style=logo instanceof HTMLImageElement?getComputedStyle(logo):null;const logoRect=logo instanceof HTMLImageElement?logo.getBoundingClientRect():null;const brandRect=brand instanceof HTMLElement?brand.getBoundingClientRect():null;
    const logoState={
      src:logo?.getAttribute('src')||'',complete:logo instanceof HTMLImageElement?logo.complete:false,
      naturalWidth:logo instanceof HTMLImageElement?logo.naturalWidth:0,naturalHeight:logo instanceof HTMLImageElement?logo.naturalHeight:0,
      decodeOk,visible:Boolean(style&&style.display!=='none'&&style.visibility!=='hidden'&&Number(style.opacity)>0&&logoRect&&logoRect.width>0&&logoRect.height>0),
      leftOfBrand:Boolean(logoRect&&brandRect&&logoRect.right<=brandRect.left+0.5),
      rect:logoRect?{left:logoRect.left,right:logoRect.right,width:logoRect.width,height:logoRect.height}:null,
      brandRect:brandRect?{left:brandRect.left,right:brandRect.right,width:brandRect.width,height:brandRect.height}:null
    };

    const wavBytes=Uint8Array.from(atob(fixtureBase64),c=>c.charCodeAt(0));
    const audioContext=new AudioContext();
    let audioBuffer;
    try{audioBuffer=await audioContext.decodeAudioData(wavBytes.buffer.slice(0));}catch(error){await audioContext.close();throw new Error('audio_decode_failed:fixture:'+error.message);}
    ...
    const store=browserAudioAssetStore();if(!store)throw new Error('indexeddb_failed:audio_store_unavailable');
    ...
    const roundTrip=await store.get(audioRef);if(!roundTrip?.blob||roundTrip.blob.size!==webmBlob.size)throw new Error('indexeddb_failed:roundtrip');
    ...
    return {isolation,logoState,mediaRecorder:{mimeType,size:webmBlob.size,durationMs:checkpoint.audioDurationMs},indexedDb:{storedSize:roundTrip.blob.size},response,storedTranscript:stored?.transcript||'',storedEngine:stored?.transcriptionEngine||null};
  },fixtureBase64);
  ...
  if(disclosure.initial.expanded!=='false'||disclosure.initial.hidden!==true||disclosure.initial.listenerType!=='function'||disclosure.open.expanded!=='true'||disclosure.open.hidden!==false||disclosure.closed.expanded!=='false'||disclosure.closed.hidden!==true)throw new Error(`disclosure_failed:${JSON.stringify(disclosure)}`);
  ...
  const transcript=String(result.storedTranscript||'').trim();if(transcript.length<8)throw new Error(`whisper_runtime_failed:empty_transcript:${JSON.stringify(result)}`);
  const normalized=transcript.normalize('NFD').replace(/\p{Diacritic}/gu,'').toLowerCase();
  const hits=['hola','prueba','sistema','reconocimiento'].filter(word=>normalized.includes(word));if(hits.length<2)throw new Error(`whisper_runtime_failed:spanish_recall(${hits.length}/4):${transcript}`);
  const offscreenPages=context.pages().filter(item=>item.url().includes(`chrome-extension://${extensionId}/offscreen.html`));
  const offscreenObserved=offscreenPages.length>0;
  if(!offscreenObserved)throw new Error('offscreen_failed:not_observed');
  ...
  console.log(`Offscreen observed: ${offscreenObserved}`);
  console.log(`SelfRelay Chromium E2E transcript: ${transcript}`);
  console.log(`Transcription engine: ${result.storedEngine}`);
}finally{
  await context.close();
