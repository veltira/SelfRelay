...
async function audioAssetMeta(serviceWorker,ref){return serviceWorker.evaluate(ref=>new Promise((resolveAsset,rejectAsset)=>{const open=indexedDB.open('selfrelay-audio',1);open.onerror=()=>rejectAsset(open.error||new Error('indexeddb_open_failed'));open.onsuccess=()=>{const db=open.result;const transaction=db.transaction('assets','readonly'),request=transaction.objectStore('assets').get(ref);request.onerror=()=>{db.close();rejectAsset(request.error||new Error('indexeddb_get_failed'));};request.onsuccess=()=>{const asset=request.result;db.close();resolveAsset(asset?{size:asset.blob.size,type:asset.blob.type,mimeType:asset.mimeType,durationMs:asset.durationMs}:null);};};}),ref);}
...
try{
  ...
  try{await captureA.locator('#save').click();}catch(error){if(!captureA.isClosed())throw error;}await captureA.waitForEvent('close',{timeout:15000}).catch(()=>{});
