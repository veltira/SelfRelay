import {unresolvedRecoveryStack} from './recovery-stack.js';
...
function render(context:any,entries:ClaimedCheckpoint[]){
  ...
  function updateSummary(){
    const active=entries.filter(entry=>!entry.released&&entry.article?.isConnected!==false);const count=active.length;
  ...
  updateSummary();document.documentElement.append(host);updateSummary();
