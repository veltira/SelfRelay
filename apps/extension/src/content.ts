...
function render(context:any,entries:ClaimedCheckpoint[]){
  ...
  shadow.querySelector<HTMLElement>('.context')!.textContent=context.members?.length?tabCountLabel(context.members.length):new URL(location.href).hostname;
...
function tabCountLabel(count:number){return count===1?'1 pestaña':`${count} pestañas`;}
