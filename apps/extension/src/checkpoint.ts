...
async function init(){
  ...
  save.onclick=async()=>{
    ...
    setBusy(true);
    const response=await chrome.runtime.sendMessage({type:'SAVE_CHECKPOINT',pendingId,text:value});
    if(!response?.ok){
      statusEl.textContent='No se pudo guardar. Probá de nuevo.';
      setBusy(false);
      return;
    }
    if(response.nextPendingId){
  ...
  skip.onclick=async()=>{
    ...
    if(!response?.ok){
      statusEl.textContent='No se pudo descartar. Probá de nuevo.';
      setBusy(false);
      return;
    }
    if(response.nextPendingId){
...
function openPending(id:string){
  location.replace(chrome.runtime.getURL(`checkpoint.html?pending=${encodeURIComponent(id)}`));
}
