...
function orderedPending(items:PendingCapture[]){
  return [...items].sort((a,b)=>a.closedAt.localeCompare(b.closedAt)||a.id.localeCompare(b.id));
}

function newestSnapshot(items:BrowserTabSnapshot[]){
  return [...items].sort((a,b)=>b.capturedAt.localeCompare(a.capturedAt)||a.tabId-b.tabId)[0]!;
}
...
export function createBackgroundController(api:ChromeApi,deps:{now?:Clock;uuid?:IdFactory}={}){
  ...
  async function refreshAllTabs(){
    for(const tab of await api.tabs.query({}))await refreshTab(tab);
  }

  async function refreshTab(tab:chrome.tabs.Tab){
    if(!tab?.id||!tab.url)return;
    let url='';
    try{url=normalizeUrl(tab.url);}catch{return;}
    ...
    if(context){
      snapshots[String(tab.id)]={tabId:tab.id,windowId:tab.windowId,contextId:context.id,url,title:tab.title||context.title||url,faviconUrl:tab.favIconUrl||context.faviconUrl,capturedAt:now()};
    }else{
  ...
  async function getActiveState(){
    ...
    let normalized='';
    try{normalized=normalizeUrl(tab.url);}catch{return{ok:true,supported:false,url:tab.url,title:tab.title||''};}
    const contexts=await storage.getContexts();
  ...
  async function trackContext(scope:BrowserContextScope){
    ...
    if(context){
      context={...context,url,origin,title:tab.title||url,faviconUrl:tab.favIconUrl||null,trackedTabId:scope==='tab'?tab.id:null,updatedAt:timestamp};
      contexts[contexts.findIndex(item=>item.id===context!.id)]=context;
    }else{
      context={id:uuid(),type:'browser',contextKey:key,scope,url,origin,title:tab.title||url,faviconUrl:tab.favIconUrl||null,trackedTabId:scope==='tab'?tab.id:null,createdAt:timestamp,updatedAt:timestamp};
  ...
  async function untrackContext(contextId:string){
    const contexts=(await storage.getContexts()).filter(item=>item.id!==contextId);
    await storage.setContexts(contexts);
    const snapshots=await storage.getSnapshots();
    for(const [key,value] of Object.entries(snapshots))if(value.contextId===contextId)delete snapshots[key];
    await storage.setSnapshots(snapshots);
    return{ok:true};
  }
  ...
  async function hasCaptureSurface(){
    try{
      const prefix=api.runtime.getURL('checkpoint.html');
      return (await api.tabs.query({})).some(tab=>typeof tab.url==='string'&&tab.url.startsWith(prefix));
    }catch{return false;}
  }
  ...
  async function openCapture(pendingId:string){
    return enqueueCapture(async()=>{
      ...
      try{
        const created=await api.windows.create({url:api.runtime.getURL(`checkpoint.html?pending=${encodeURIComponent(pendingId)}`),type:'popup',width:460,height:560,focused:true});
        captureWindowId=created?.id??null;
        return true;
      }catch{return false;}
    });
  }

  async function openOldestPendingCapture(){
    const first=orderedPending(await storage.getPending())[0];
    return first?openCapture(first.id):false;
  }

  async function clearTrackedTabBindings(contexts:BrowserContext[],snapshots:BrowserTabSnapshot[]){
    const removedTabIds=new Set(snapshots.map(item=>item.tabId));
    let changed=false;
    for(let i=0;i<contexts.length;i++){
      const context=contexts[i]!;
      if(context.scope==='tab'&&context.trackedTabId!==null&&removedTabIds.has(context.trackedTabId)){
        contexts[i]={...context,trackedTabId:null,updatedAt:now()};
        changed=true;
      }
    }
    if(changed)await storage.setContexts(contexts);
  }

  async function processWindowClose(tabId:number,windowId:number){
    const snapshots=await storage.getSnapshots();
    const trigger=snapshots[String(tabId)];
    if(!trigger)return{ok:true,created:0,pending:[] as PendingCapture[]};

    const windowSnapshots=Object.values(snapshots).filter(item=>item.windowId===windowId);
    const closing=windowSnapshots.length?windowSnapshots:[trigger];
    for(const item of closing)delete snapshots[String(item.tabId)];
    await storage.setSnapshots(snapshots);

    const contexts=await storage.getContexts();
    await clearTrackedTabBindings(contexts,closing);
    const knownContexts=new Set(contexts.map(item=>item.id));
    const byContext=new Map<string,BrowserTabSnapshot[]>();
    for(const item of closing){
      if(!knownContexts.has(item.contextId))continue;
      const group=byContext.get(item.contextId)||[];
      group.push(item);
      byContext.set(item.contextId,group);
    }

    const created:PendingCapture[]=[];
    const pending=await storage.getPending();
    for(const [contextId,items] of byContext){
      const snapshot=newestSnapshot(items);
      const capture:PendingCapture={id:uuid(),contextId,url:snapshot.url,title:snapshot.title,closedAt:now()};
      pending.push(capture);
      created.push(capture);
    }
    if(created.length)await storage.setPending(pending);
  ...
  async function processNormalTabClose(tabId:number){
    ...
    if(!snapshot)return{ok:true,created:false,pending:null};
    delete snapshots[String(tabId)];
    await storage.setSnapshots(snapshots);
    const contexts=await storage.getContexts();
    const context=contexts.find(item=>item.id===snapshot.contextId);
    if(!context)return{ok:true,created:false,pending:null};
    await clearTrackedTabBindings(contexts,[snapshot]);
    const pending:PendingCapture={id:uuid(),contextId:context.id,url:snapshot.url,title:snapshot.title,closedAt:now()};
    const all=await storage.getPending();
    all.push(pending);
    await storage.setPending(all);
    await openCapture(pending.id);
    return{ok:true,created:true,pending};
  }

  function handleRemoved(tabId:number,isWindowClosing:boolean,windowId?:number){
    return enqueueRemoval(()=>isWindowClosing&&typeof windowId==='number'?processWindowClose(tabId,windowId):processNormalTabClose(tabId));
  }

  async function handleStartup(){
    captureWindowId=null;
    await refreshAllTabs();
    return openOldestPendingCapture();
  }

  function handleWindowRemoved(windowId:number){
    if(captureWindowId===windowId)captureWindowId=null;
  }

  async function getPendingCapture(id:string){
    const pending=(await storage.getPending()).find(item=>item.id===id);
    if(!pending)return{ok:false,error:'pending_not_found'};
    const context=(await storage.getContexts()).find(item=>item.id===pending.contextId)||null;
    return{ok:true,pending,context};
  }

  async function removePending(id:string){
    const remaining=(await storage.getPending()).filter(item=>item.id!==id);
    await storage.setPending(remaining);
    return orderedPending(remaining)[0]?.id??null;
  }

  async function discardPending(id:string){
    const exists=(await storage.getPending()).some(item=>item.id===id);
    if(!exists)return{ok:false,error:'pending_not_found'};
    return{ok:true,nextPendingId:await removePending(id)};
  }

  async function saveCheckpoint(pendingId:string,text:string){
    const clean=text.replace(/\u0000/g,'').trim().slice(0,12000);
    if(!clean)return{ok:false,error:'empty_checkpoint'};
    const pending=(await storage.getPending()).find(item=>item.id===pendingId);
    if(!pending)return{ok:false,error:'pending_not_found'};
    const checkpoint:Checkpoint={id:uuid(),contextId:pending.contextId,originalText:clean,createdAt:now(),resolvedAt:null};
    const all=await storage.getCheckpoints();
    all.push(checkpoint);
    await storage.setCheckpoints(all);
    const nextPendingId=await removePending(pendingId);
    return{ok:true,checkpoint,nextPendingId};
  }

  async function lookupCheckpoint(rawUrl:string,tabId?:number){
    let url='';
    try{url=normalizeUrl(rawUrl);}catch{return{ok:true,checkpoint:null,context:null};}
    const context=findBestContext(await storage.getContexts(),url,tabId);
    if(!context)return{ok:true,checkpoint:null,context:null};
    return{ok:true,checkpoint:(await storage.unresolvedFor(context.id))[0]||null,context};
  }

  async function resolveCheckpoint(id:string){
    const all=await storage.getCheckpoints();
    const index=all.findIndex(item=>item.id===id);
    if(index<0)return{ok:false,error:'checkpoint_not_found'};
    all[index]={...all[index]!,resolvedAt:now()};
    await storage.setCheckpoints(all);
    return{ok:true};
  }

  async function getContextHistory(contextId:string){
    const context=(await storage.getContexts()).find(item=>item.id===contextId)||null;
    const checkpoints=(await storage.getCheckpoints()).filter(item=>item.contextId===contextId).sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
    return{ok:true,context,checkpoints};
  }
