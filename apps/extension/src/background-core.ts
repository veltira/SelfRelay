...
export function createBackgroundController(api:ChromeApi,deps:{now?:Clock;uuid?:IdFactory}={}){
  ...
  function handleRemoved(tabId:number,isWindowClosing:boolean,windowId?:number){return enqueueRemoval(()=>isWindowClosing&&typeof windowId==='number'?processWindowClose(tabId,windowId):processNormalTabClose(tabId));}
