...
if ($Install.ExitCode -ne 0) { throw "0.2.4 silent install failed with $($Install.ExitCode)" }
$Legacy.Refresh()
if (-not $Legacy.HasExited) { throw "0.2.4 installer left the old SelfRelay.exe process running" }
...
# Real 0.2.4 startup must preserve the exact 0.2.1 selection, settings,
...
if (-not $MigrationOk) { throw "0.2.4 did not preserve 0.2.1 selections, settings, worksets, checkpoint and pending-capture data" }
