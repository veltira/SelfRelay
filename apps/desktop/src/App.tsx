...
function HistoryCheckpoint({ item, onOpen }: { item: CheckpointRecord; onOpen: (id: number) => Promise<void> }) {
...
function QaPreview({ state }: { state: string }) {
  ...
  const settings: SettingsView = { launchAtStartup: true, trackingActive: true, archiveResolvedCheckpoints: true, version: PRODUCT_VERSION, dataDirectory: "C:\\Users\\Demo\\AppData\\Local\\SelfRelay" };
  return <main className="app-shell"><header className="app-header"><Brand /><nav className="nav-tabs"><button className={section === "apps" ? "active" : ""}>Aplicaciones</button><button className={section === "worksets" ? "active" : ""}>Entornos</button><button className={section === "history" ? "active" : ""}>Historial</button><button className={section === "settings" ? "active" : ""}>Ajustes</button></nav><span className="tracking-pill active"><span />Activo</span></header>{section === "apps" && <ApplicationsSection discovered={qaApps} tracked={qaTracked} busy={false} onAdd={async () => undefined} onRemove={async () => undefined} onPick={async () => undefined} onManualCheckpoint={async () => undefined} />}{section === "worksets" && <WorksetsSection worksets={qaWorksets} tracked={qaTracked} onCreate={async () => undefined} onRename={async () => undefined} onMembers={async () => undefined} onDelete={async () => undefined} />}{section === "history" && <HistorySection history={qaHistory} worksets={qaWorksets} onOpen={async () => undefined} />}{section === "settings" && <SettingsSection settings={settings} onStartup={async () => undefined} onPause={async () => undefined} onArchive={async () => undefined} />}</main>;
}
