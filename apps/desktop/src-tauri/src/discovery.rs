...
fn collect_shortcuts(root: &Path, output: &mut Vec<PathBuf>) {
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            ...
            .map(|value| value.eq_ignore_ascii_case("lnk")).unwrap_or(false)
        {
            output.push(path);
        }
    }
...
pub(crate) fn resolve_shortcut(path: &Path) -> Option<String> {
    use ::windows::{core::{Interface, PCWSTR}, Win32::{System::Com::{
...
fn packaged_applications() -> Vec<DiscoveredApplication> {
    use ::windows::{core::Interface, Win32::{
        ...
        UI::Shell::{BHID_EnumItems, FOLDERID_AppsFolder, IEnumShellItems, IShellItem, IShellItem2,
            KNOWN_FOLDER_FLAG, SHGetKnownFolderItem, SIGDN_NORMALDISPLAY},
    }};
    unsafe {
        let initialized = CoInitializeEx(None, COINIT_APARTMENTTHREADED).is_ok();
        let result = (|| -> ::windows::core::Result<Vec<DiscoveredApplication>> {
            ...
            loop {
                ...
                result.push(DiscoveredApplication {
                    ...
                    application_name: display.clone(),
...
fn uninstall_entries(root: ::windows::Win32::System::Registry::HKEY) -> Vec<DiscoveredApplication> {
    ...
    for view in [KEY_WOW64_64KEY, KEY_WOW64_32KEY] {
        ...
        for subkey in enum_subkeys(key) {
            ...
            if let Some(candidate) = candidate_from_executable(path.clone(), display_name.clone(), vec![display_name, subkey, path]) {
                result.push(candidate);
            }
        }
        unsafe { let _ = ::windows::Win32::System::Registry::RegCloseKey(key); }
...
fn normalize_display_icon(value: &str) -> String {
    ...
    let without_index = trimmed.rsplit_once(',')
        .map(|(left, right)| if right.trim().parse::<i32>().is_ok() { left } else { trimmed })
        .unwrap_or(trimmed);
...
fn read_registry_string(
    ...
    let mut bytes = 0u32;
    if unsafe { RegQueryValueExW(key, value, None, Some(&mut kind), None, Some(&mut bytes)) }.is_err()
        || bytes == 0 || (kind != REG_SZ && kind != REG_EXPAND_SZ)
    {
    ...
    if kind == REG_EXPAND_SZ { value = expand_environment(&value); }
    Some(value)
}
...
mod tests {
    ...
    #[test]
    fn duplicate_executable_names_do_not_collapse() {
        let a = identity::application_id(Some(r"C:\Apps\A\same.exe"), "same.exe", None);
        let b = identity::application_id(Some(r"D:\Apps\B\same.exe"), "same.exe", None);
        assert_ne!(a, b);
    }

    #[test]
    fn unsupported_selection_reports_clear_reason() {
        let result = tracked_from_selection("definitely-not-an-application.txt");
        assert!(result.unwrap_err().contains("No pudimos seguir esta aplicación todavía"));
    }
    ...
    fn manual_lnk_resolution_uses_shell_link() {
        ...
        let temp = std::env::temp_dir().join(format!("selfrelay-link-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&temp);
        let target = std::env::current_exe().unwrap();
        let link_path = temp.join("SelfRelay fixture link.lnk");
        unsafe {
            let initialized = CoInitializeEx(None, COINIT_APARTMENTTHREADED).is_ok();
            let link: IShellLinkW = CoCreateInstance(&ShellLink, None, CLSCTX_INPROC_SERVER).unwrap();
            let target_wide = wide(target.to_string_lossy().as_ref());
            link.SetPath(PCWSTR(target_wide.as_ptr())).unwrap();
            let persist: IPersistFile = link.cast().unwrap();
            let link_wide = wide(link_path.to_string_lossy().as_ref());
            persist.Save(PCWSTR(link_wide.as_ptr()), true).unwrap();
            if initialized { CoUninitialize(); }
        }
        let resolved = resolve_shortcut(&link_path).unwrap();
        assert_eq!(identity::normalized_path(&resolved), identity::normalized_path(target.to_string_lossy().as_ref()));
    ...
    fn packaged_app_discovery_is_safe_when_runner_exposes_appsfolder() {
        for app in packaged_applications() {
            assert!(app.application_id.starts_with("aumid:"));
            assert!(app.app_user_model_id.is_some());
        }
    }
}
